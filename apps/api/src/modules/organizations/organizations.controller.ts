...
export class OrganizationsController {
  ...
  invite(@CurrentUser() currentUser: { activeOrganizationId: string }, @Body() body: unknown) {
    const parsed = z.object({ email: z.email(), firstName: z.string().trim().min(1).max(100), lastName: z.string().trim().min(1).max(100), roleId: z.uuid() }).safeParse(body);
    if (!parsed.success) throw new BadRequestException(parsed.error.flatten());
    return this.auth.invite(currentUser.activeOrganizationId, parsed.data);
  }
}
