...
| 1 | Monorepo foundation | 0 | pnpm/Turbo workspace, API/web apps, Docker Postgres, environment template, health endpoint, lint/type/build checks |
| 2 | Database foundation | 1 | Prisma package/client, local and Neon `DATABASE_URL` configuration, migration workflow |
...
Create only the workspace and development path. The API responds to `GET /health`; the web app renders a minimal shell; Docker Compose runs PostgreSQL; `.env.example` documents variables without values. Share TypeScript/lint/format configuration through `packages/config`.
...
Start Phase 1 after approval. Expected files are root workspace configuration, `apps/api`, `apps/web`, `packages/config`, `infra/docker`, `.env.example`, and Docker Compose configuration. The result is a runnable foundation with no business schema or feature code.
